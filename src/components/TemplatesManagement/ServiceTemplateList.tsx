...
export default function ServiceTemplateList() {
  ...
  const [selectedTemplate, setSelectedTemplate] = useState<ServiceTemplate | undefined>();
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center h-64">
        <div className="text-slate-400">Lädt Zusatzleistungen...</div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-white">Zusatzleistungen</h2>
          <p className="text-slate-400 mt-1">Vordefinierte Services für Buchungen</p>
        </div>
        <button
          onClick={handleCreate}
          className="flex items-center gap-2 px-4 py-3 bg-emerald-600 hover:bg-emerald-700 text-white font-semibold rounded-lg transition-colors"
        >
      ...
      {templates.length === 0 ? (
        <div className="text-center py-16">
          ...
          <p className="text-slate-500 mb-6">
            Erstelle Zusatzleistungen, die du bei Buchungen hinzufügen kannst
          </p>
          <button
            onClick={handleCreate}
      ...
      <ConfirmDialog
        ...
        message="Möchtest du diese Zusatzleistung wirklich löschen? Diese Aktion kann nicht rückgängig gemacht werden."
        confirmText="Löschen"
