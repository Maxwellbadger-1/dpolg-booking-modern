...
Write-Host "`n🚀 Starting Quick Release for v$Version" -ForegroundColor Cyan
Write-Host ""
...
Write-Host "🏗️  Step 4/7: Building release binaries (this takes ~5-10 minutes)..." -ForegroundColor Yellow
...
$ReleaseBody = @{
    ...
    body = "## 🎉 Änderungen in v$Version`n`n- ✅ [Beschreibung der Änderungen hier einfügen]`n`n## 📥 Installation`n**Windows:** Laden Sie die ``-setup.exe`` Datei herunter und installieren Sie die App.`n`n## 🔄 Auto-Update`nWenn Sie bereits eine ältere Version installiert haben, wird automatisch ein Update-Dialog angezeigt."
    draft = $false
    prerelease = $false
} | ConvertTo-Json
...
# Step 7: Upload files
Write-Host "⬆️  Step 7/7: Uploading files to GitHub..." -ForegroundColor Yellow
...
Write-Host ""
Write-Host "🎉 Release v$Version erfolgreich veröffentlicht!" -ForegroundColor Cyan
Write-Host ""
Write-Host "🔗 Release URL: https://github.com/$Repo/releases/tag/v$Version" -ForegroundColor Blue
Write-Host ""
Write-Host "🧪 Nächster Schritt: Auto-Update testen" -ForegroundColor Yellow
Write-Host "   1. Öffnen Sie die installierte App (ältere Version)"
